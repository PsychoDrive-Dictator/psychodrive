...
void drawGuyStatusWindow(const char *windowName, Guy *pGuy)
{
    ...
    *pGuy->getInputIDPtr() = atoi(vecInputLabels[*pGuy->getInputListIDPtr()].c_str());
    ...
    ImGui::Text("airborne %i poseStatus %i actionStatus %i jumpStatus %i landingAdjust %i", pGuy->getAirborne(), pGuy->getPoseStatus(), pGuy->getActionStatus(), pGuy->getJumpStatus(), pGuy->getLandingAdjust());
    ...
    if (pGuy->getProjectile()) {
        ...
        ImGui::Text("health %i unique %i install %i", pGuy->getHealth(), pGuy->getUniqueParam(), pGuy->getInstallFrames());
