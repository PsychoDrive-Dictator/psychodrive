...
void drawGuyStatusWindow(const char *windowName, Guy *pGuy)
{
    ...
    *pGuy->getInputIDPtr() = atoi(vecInputLabels[*pGuy->getInputListIDPtr()].c_str());
    ...
    if (pGuy->getProjectile()) {
        ...
        ImGui::Text("health %i unique %s debuff %i style %i install %i", pGuy->getHealth(), pGuy->getUniqueParam().c_str(), pGuy->getDebuffTimer(), pGuy->getStyle(), pGuy->getInstallFrames());
